...
class catch_page extends StatefulWidget {
  final Map<String, dynamic>? ccatch;
  const catch_page({super.key, this.ccatch});

  @override
  State<catch_page> createState() => _catch_pageState();
}

class _catch_pageState extends State<catch_page> {
  ...
  void initState() {
    super.initState();
    if (widget.ccatch != null) {
      desController.text = widget.ccatch!["description"] ?? "";
      byController.text = widget.ccatch!["by"] ?? '';
      moneyController.text = widget.ccatch!["money"] ?? '';
      dateController.text = widget.ccatch!['date'] ?? '';
      timeController.text = widget.ccatch!["time"] ?? '';
    }
  }
  ...
  Future<void> syncOffline() async {
    var box = await Hive.openBox("catch");
    if (await isConnected()) {
      CollectionReference expensesCollection = FirebaseFirestore.instance.collection("catch");
  ...
  Future<void> saveToFirebase() async {
    ...
    if (user != null) {
      ...
      if (await isConnected()) {
        ...
        ;
        if (widget.ccatch != null) {
          await expensesCollection.doc(widget.ccatch!['time']).update(
              expenseData);
        } else {
