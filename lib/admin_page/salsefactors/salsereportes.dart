...
class sells_table_reports extends StatefulWidget {
  const sells_table_reports({super.key});

  @override
  State<sells_table_reports> createState() => _sells_table_reportsState();
}

class _sells_table_reportsState extends State<sells_table_reports> {
  DateTime? _startdate;
  DateTime? _enddate;
      ...
      body: StreamBuilder<QuerySnapshot>(
        ...
        builder: (context, snapshot) {
          ...
          return SingleChildScrollView(
            ...
            child: DataTable(
              border: TableBorder.all(borderRadius: BorderRadius.circular(8),color: Colors.blueAccent,width: 1),
              columns: [
              ...
              rows: filteredDocs.map((doc) {
                final factors = doc.data() as Map<String, dynamic>;
                final total = (factors["total"] ?? 0).toString();
                final totalPay = (factors["totalpay"] ?? 0).toString();

                return DataRow(
                  cells: [
                    DataCell(
                      Text(factors["name"] ?? "", style: TextStyle(fontSize: 12)),
                    ),
                    DataCell(
                      Text(
                    ...
                    DataCell(
                      Text(factors["totalpay"] ?? "", style: TextStyle(fontSize: 12)),
                    ),
                    DataCell(
                      Text(factors["date"] ?? "", style: TextStyle(fontSize: 12)),
                    ),
                  ],
                );
              }).toList(),
